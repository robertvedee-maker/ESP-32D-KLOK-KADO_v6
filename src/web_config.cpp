...
void initWebServer()
{
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request)
        ...
        html += ".card{background:#1E1E1E;padding:20px;border-radius:10px;display:block;margin:20px auto 0;border:1px solid #333;width:85%;max-width:400px; box-sizing: border-box;}";
        ...
        // html += "<div style='margin-top:40px; border-top:1px solid #444; padding-top:20px;'>";
        // html += "<label style='color:#FF5555; display:block; margin-bottom:10px;'>";
        // html += "<input type='checkbox' onchange='document.getElementById(\"btnReset\").disabled = !this.checked'> Ik begrijp dat dit alles wist</label>";
        // html += "<form action='/reset' method='POST'>";
        // html += "<button id='btnReset' type='submit' disabled style='background:#444; color:#888; padding:10px;'>FABRIEKSINSTELLINGEN</button>";
        // html += "</form></div>";
        // html += "<details style='margin-top:40px; color:#FF5555;'>";
        // html += "<summary style='cursor:pointer; font-weight:bold;'>GEVARENZONE (Klik om te openen)</summary>";
        // html += "<p>Hiermee wis je alle instellingen!</p>";
        // html += "<form action='/reset' method='POST'><button style='background:#AA0000;'>NU RESETTEN</button></form>";
        // html += "</details>";
        html += "<div class='card'>";
        html +=   "<details style='background:#1a1a1a; padding:15px; border-radius:8px; border:1px solid #333;'>";
        html +=     "<summary style='color:#FF5555; cursor:pointer; font-weight:bold; font-size:1.1em;'>";
        html +=       "RESET (Klik om te openen)";
        html +=     "</summary>";
        html +=     "<div style='margin-top:15px;'>";
        html +=       "<p style='color:#ccc; font-size:0.9em;'>Hiermee wis je alle opgeslagen gegevens en start de klok opnieuw op in Setup-modus.</p>";
        html +=       "<form action='/reset' method='POST' onsubmit='return confirm(\"WEET JE HET ZEKER? Dit kan niet ongedaan worden gemaakt!\")'>";
        html +=         "<button type='submit' style='background:#AA0000; color:white; padding:12px 25px; border:none; border-radius:5px; width:100%; cursor:pointer; font-weight:bold;'>";
        html +=           "NU FABRIEKSINSTELLINGEN HERSTELLEN";
        html +=         "</button>";
        html +=       "</form>";
        html +=     "</div>";
        html +=   "</details>";
    ...
    server.on("/save", HTTP_POST, [](AsyncWebServerRequest *request)
              {
                ...
                //       currentIP = "192.168.4.1";
                  String redirectURL;
                  if (state.network.mdns.length() > 0)
                  {
                      redirectURL = "http://" + state.network.mdns + ".local/";
                  }
                  else
                  {
                      // Fallback naar het AP-adres als mDNS niet gezet is
                ...
                //   html += "<p>Naam: <b>" + state.network.mdns + ".local</b></p>";
                  ...
                  Serial.print(F("SSID: "));
                  Serial.println(state.network.ssid);
                  Serial.print(F("mDNS: "));
                  Serial.println(state.network.mdns);
